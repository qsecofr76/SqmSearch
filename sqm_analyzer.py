...
Supporto a cataloghi multipli di aree (es. NordEst, Lombardia, ecc.).
"""
...
def load_catalog(cat_id: str = "nordest") -> List[Dict]:
    """Carica un catalogo da file JSON o usa la copia incorporata come fallback."""
    cdir = get_catalogs_dir()
    cat_file = os.path.join(cdir, f"{cat_id}.json")
    if os.path.exists(cat_file):
        try:
            ...
            pass
    if cat_id.lower() == "nordest":
        return NORDEST_SITES
    return []

CURATED_SITES = load_catalog("nordest")
...
def calculate_routes_for_origin(origin_lat: float, origin_lon: float, origin_name: str = "", sites: Optional[List[Dict]] = None) -> List[Dict]:
    ...
    coords = [f"{origin_lon:.5f},{origin_lat:.5f}"] + [f"{s['lon']:.5f},{s['lat']:.5f}" for s in sites]
    coord_str = ';'.join(coords)
    dest_indices = ';'.join(str(i) for i in range(1, len(sites) + 1))
